...
def create_local_agent_app() -> Flask:
    ...
    def team_dashboard_monthly_report_draft():
        ...
        result = service.generate_draft(
            ...
            period_end_exclusive=str(payload.get("period_end_exclusive") or "") or None,
    ...
    @app.post("/api/local-agent/meeting-recorder/signal-check")
    def meeting_recorder_signal_check():
        payload = request.get_json(silent=True) or {}
        record = _get_meeting_recorder_runtime().check_recording_signal(
            record_id=str(payload.get("record_id") or ""),
            owner_email=str(payload.get("owner_email") or ""),
        )
        return jsonify({"status": "ok", "record": _meeting_record_summary(record)})
    ...
    def meeting_recorder_process_async():
        ...
        try:
            result = _queue_meeting_recorder_process_job(
                ...
                record_id=str(payload.get("record_id") or ""),
                owner_email=str(payload.get("owner_email") or ""),
                send_email_on_complete=bool(payload.get("send_email_on_complete")),
            )
            return jsonify(result)
        except ToolError as error:
...
def _summarize_bpmis_proxy_args(operation: str, args: list[Any]) -> dict[str, Any]:
    ...
    if operation == "list_jira_tasks_created_by_emails" and args and isinstance(args[0], list):
        ...
        summary["keyword_present"] = bool(str(args[0] or "").strip())
    elif operation == "get_jira_ticket_details" and args and isinstance(args[0], list):
        summary["ticket_count"] = len(args[0])
    ...
    elif operation == "update_biz_project_status":
        summary["project_issue_id_present"] = bool(args and str(args[0] or "").strip())
        summary["status_present"] = bool(len(args) > 1 and str(args[1] or "").strip())
    elif operation == "list_actual_mandays_for_projects" and args and isinstance(args[0], list):
        summary["project_count"] = len(args[0])
...
def _send_meeting_recorder_minutes_email_after_process(
    *,
    ...
    try:
        ...
        current_app.logger.warning(
            "Meeting Recorder local-agent auto email failed. record_id=%s",
            record_id,
            exc_info=True,
        )
...
def _run_meeting_recorder_process_job(
    ...
    with app.app_context():
        ...
        try:
            ...
            completed_record = _meeting_record_for_owner(record_id=record_id, owner_email=owner_email)
