...
class SourceCodeQAService:
    ...
    def _runtime_evidence_prompt_section(cls, runtime_evidence: list[dict[str, Any]]) -> str:
        ...
        lines = [
            ...
            "- For AF and GRC country-scoped SQL questions, first explain the actual SQL/table logic from code and data dictionary evidence, including the chosen table names, key filters, joins, reviewer/status rows, and timestamp assumptions. Then add the country DB routing note only as an execution assumption.",
            "- For AF and GRC country-scoped SQL questions, assume the selected country points to that country's separate runtime DB instance. Do not invent a country filter, cross-country union, or shared physical database unless repository/runtime evidence proves it. This DB-routing rule is not a complete answer by itself.",
