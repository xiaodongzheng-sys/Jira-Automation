...
class TeamDashboardVersionPlanTest(unittest.TestCase):
    ...
    def test_sync_builds_active_bundle_seen_versions_and_synced_rows(self) -> None:
        ...
        self.assertIn("sabrina.chan@npt.sg", client.release_window_calls[0]["emails"])
        self.assertIn("weilin.ang@npt.sg", client.release_window_calls[0]["emails"])
    ...
    def test_parser_market_and_date_helper_edges(self) -> None:
        ...
        self.assertEqual(vplan._normalize_pm_values(["sabrina.chan@npt.sg"]), ["Sabrina"])
        self.assertEqual(vplan._normalize_pm_values(["weilin.ang@npt.sg"]), ["Wei Lin"])
        self.assertFalse(vplan._is_sabrina_grc_ticket({"reporter": {"email": "sabrina.chan@npt.sg"}, "summary": "AF Rule Review"}))
        self.assertTrue(vplan._is_sabrina_grc_ticket({"reporter": {"email": "sabrina.chan@npt.sg"}, "summary": "GRC Phase 1 Enhancement"}))
