...
class GmailSeaTalkDemoRouteTests(unittest.TestCase):
    ...
    def test_owner_can_open_seatalk_summary_page(self):
        ...
        self.assertIn(b"data-seatalk-project-updates", response.data)
        self.assertIn(b"Project update product lines", response.data)
        self.assertIn(b"Anti-fraud", response.data)
        self.assertIn(b"Credit Risk", response.data)
        self.assertIn(b"Ops Risk", response.data)
