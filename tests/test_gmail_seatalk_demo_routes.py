...
class GmailSeaTalkDemoRouteTests(unittest.TestCase):
    ...
    def test_owner_no_longer_sees_seatalk_management_tab_on_index(self):
        ...
        self.assertLess(html.index(">Source Code<"), html.index(">Meetings<"))
        self.assertLess(html.index(">Meetings<"), html.index(">PRDs<"))
        self.assertLess(html.index(">PRDs<"), html.index(">Projects<"))
        self.assertLess(html.index("Team Dashboard"), html.index("Reports"))
        self.assertLess(html.index("Reports"), html.index("BPMIS Automation Tool"))
    ...
    def test_owner_seatalk_summary_page_redirects_to_reports_name_mapping(self):
        with self.app.test_client() as client:
            self._login_owner(client, scopes=[GMAIL_READONLY_SCOPE])
            response = client.get("/gmail-sea-talk-demo", follow_redirects=False)

        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.headers["Location"], "/reports?tab=seatalk-name-mapping")
