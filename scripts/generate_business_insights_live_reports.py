    ...
    *,
    ...
    if report_id == AF_RULES_FEATURES_REPORT_ID:
        ...
        if treatment_coverage:
            ...
            treatment_idx = _coverage_col("treatment_type", "treatment_tag", "treatment_name", "treatment")
            source_idx = _coverage_col("source_table", "source_db")
            rule_idx = _coverage_col("rule_id")
            two_way_template_idx = _coverage_col("two_way_template_id")
            one_way_template_idx = _coverage_col("one_way_template_id")
            pt_date_idx = _coverage_col("pt_date")
            scenario_idx = _coverage_col("scenario_group_name", "scenario_name", "l1_scene_name", "l2_enum_name")

            coverage_kpis: list[tuple] = []
            if tr:
                coverage_kpis.append(("Config rows", _format_number(len(tr))))
                if rule_idx is not None:
                    distinct_rules = {
                        str(row[rule_idx]).strip()
                        for row in tr
                        if rule_idx < len(row) and str(row[rule_idx]).strip()
                    }
                    coverage_kpis.append(("Rules covered", _format_number(len(distinct_rules))))
                if treatment_idx is not None:
                    distinct_treatments = {
                        str(row[treatment_idx]).strip()
                        for row in tr
                        if treatment_idx < len(row) and str(row[treatment_idx]).strip()
                    }
                    coverage_kpis.append(("Treatment types", _format_number(len(distinct_treatments))))
                if pt_date_idx is not None:
                    dates = sorted({
                        str(row[pt_date_idx]).strip()
                        for row in tr
                        if pt_date_idx < len(row) and str(row[pt_date_idx]).strip()
                    })
                    if dates:
                        coverage_kpis.append(("Latest partition", dates[-1]))
                if coverage_kpis:
                    table_panels.append(_kpi_cards_panel("Rule Treatment Config Coverage Summary", coverage_kpis))

            if tr:
                coverage_note = (
                    "Google Sheet tab 2_rule_treatment_config_coverage. This view summarizes the current "
                    "rule_treatment_config coverage by configured treatment path and template linkage; use "
                    "the table below for row-level rule lookup."
                )
                status_counts = _coverage_counts(status_idx)
                if status_counts:
                    status_bar = _bar_panel(
                        "Rule Treatment Config Coverage by Status",
                        sorted(status_counts.items(), key=lambda kv: _number(kv[1]), reverse=True)[:15],
                        note=coverage_note,
                    )
                    if status_bar:
                        table_panels.append(status_bar)
                treatment_counts = _coverage_counts(treatment_idx)
                if treatment_counts:
                    treatment_bar = _bar_panel(
                        "Rule Treatment Config Coverage by Treatment Type",
                        sorted(treatment_counts.items(), key=lambda kv: _number(kv[1]), reverse=True)[:15],
                        note=coverage_note,
                    )
                    if treatment_bar:
                        table_panels.append(treatment_bar)
                template_counts: dict[str, float] = {}
                for row in tr:
                    has_two_way = (
                        two_way_template_idx is not None
                        and two_way_template_idx < len(row)
                        and row[two_way_template_idx] not in (None, "")
                    )
                    has_one_way = (
                        one_way_template_idx is not None
                        and one_way_template_idx < len(row)
                        and row[one_way_template_idx] not in (None, "")
                    )
                    if has_two_way and has_one_way:
                        key = "Two-way + one-way templates"
                    elif has_two_way:
                        key = "Two-way template linked"
                    elif has_one_way:
                        key = "One-way template linked"
                    elif two_way_template_idx is not None or one_way_template_idx is not None:
                        key = "No template linked"
                    else:
                        key = ""
                    if key:
                        template_counts[key] = template_counts.get(key, 0.0) + 1
                if template_counts:
                    template_bar = _bar_panel(
                        "Rule Treatment Config Coverage by Template Linkage",
                        sorted(template_counts.items(), key=lambda kv: _number(kv[1]), reverse=True),
                        note="Counts treatment-config rows by whether they are linked to two-way and/or one-way notification templates.",
                    )
                    if template_bar:
                        table_panels.append(template_bar)
                source_counts = _coverage_counts(source_idx)
                if len(source_counts) > 1:
                    source_bar = _bar_panel(
                        "Rule Treatment Config Coverage by Source Table",
                        sorted(source_counts.items(), key=lambda kv: _number(kv[1]), reverse=True)[:15],
                        note="Source table distribution for the Google Sheet coverage extract.",
                    )
                    if source_bar:
                        table_panels.append(source_bar)
                if not (status_counts or treatment_counts or template_counts or source_counts):
                    scenario_counts = _coverage_counts(scenario_idx)
                    if scenario_counts:
                        scenario_bar = _bar_panel(
                            "Rule Treatment Config Coverage by Scenario",
                            sorted(scenario_counts.items(), key=lambda kv: _number(kv[1]), reverse=True)[:15],
                            note=coverage_note,
                        )
                        if scenario_bar:
                            table_panels.append(scenario_bar)
            table_panels.append(
                _searchable_table_panel(
